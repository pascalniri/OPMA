...
const overallProgress = Math.round(
  MILESTONES.reduce((s, m) => s + (m.weight / 100) * m.progress, 0),
);
...
const METRICS = [
  {
    ...
    bar: timelineProgress,
    sub: `${CURRENT_PROJECT.startDate} → ${CURRENT_PROJECT.endDate}`,
    status: getStatus(timelineProgress),
    trend: timelineProgress > 50 ? "warn" : "ok",
  },
  {
    icon: DollarSign,
    label: "Budget",
    value: `${budgetPct}%`,
    bar: budgetPct,
    sub: `$${(CURRENT_PROJECT.budget.used / 1000).toFixed(0)}k of $${(CURRENT_PROJECT.budget.total / 1000).toFixed(0)}k`,
    status: getStatus(budgetPct, true),
    trend: budgetPct > 70 ? "warn" : "ok",
  },
  {
    icon: Zap,
    ...
    trend: overallProgress > 60 ? "ok" : "warn",
  },
  {
    icon: Users2,
    label: "Risk Exposure",
    value: `${atRiskCount} items`,
    bar: (atRiskCount / MILESTONES.length) * 100,
    sub: `${atRiskCount} milestones at risk`,
    status: (atRiskCount === 0
      ? "good"
      : atRiskCount > 2
        ? "bad"
        : "warn") as HealthStatus,
    trend: atRiskCount > 2 ? "warn" : "ok",
  },
];
      ...
      {METRICS.map((m) => {
        ...
        return (
          ...
          >
            ...
            <div
              className="text-[26px] font-bold tabular-nums leading-none mb-3"
              style={{ color: p.color }}
            >
              {m.value}
            </div>
            ...
            >
              <div
                ...
                style={{ width: `${m.bar}%`, background: p.bar }}
              />
            </div>
            <p className="text-[11px] text-white/30 font-medium leading-snug">
              {m.sub}
            </p>
          </div>
        );
