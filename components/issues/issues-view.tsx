...
const STATUS_CONFIG: Record<ActivityStatus, { 
  ...
  "todo": { label: "Backlog", color: "text-slate-400", bg: "bg-slate-50", icon: Circle, boardColumn: "Backlog", order: 0 },
  "in-progress": { label: "In Progress", color: "text-foreground", bg: "bg-black/[0.04]", icon: Clock, boardColumn: "In Progress", order: 1 },
  "pending-verification": { label: "In Review", color: "text-orange-500", bg: "bg-orange-50", icon: AlertCircle, boardColumn: "In Review", order: 2 },
};
...
const PRIORITY_CONFIG: Record<Priority, { label: string; color: string; bg: string; order: number }> = {
  ...
  "medium": { label: "Medium", color: "text-foreground", bg: "bg-black/[0.04]", order: 1 },
  "high": { label: "High", color: "text-orange-500", bg: "bg-orange-50", order: 2 },
  "critical": { label: "Urgent", color: "text-rose-500", bg: "bg-rose-50", order: 3 },
};
...
export function IssuesView({ title, description }: IssuesViewProps) {
  ...
  }, [searchQuery, selectedStatus, selectedProject, selectedPriority, selectedAssignee]);
  ...
  // ── Components ──
  const IssueCard = ({ issue, isSelected }: { issue: Issue; isSelected: boolean }) => {
    ...
    return (
      <div 
        className={cn(
          ...
          isSelected ? "border-foreground/50 bg-black/[0.02]" : "border-black/10 hover:border-black/15",
          "rounded p-4"
        )}
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0 space-y-2">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-[10px] font-bold text-muted-foreground/60 uppercase tracking-wider">{issue.id}</span>
              <span className={cn("px-1.5 py-0.5 rounded-[2px] text-[9px] font-bold uppercase tracking-tight", priority.bg, priority.color)}>
                {priority.label}
              </span>
            ...
            <div className="flex items-center gap-4 pt-1">
              ...
              <div className={cn("flex items-center gap-1 px-2 py-0.5 rounded-[4px] text-[9px] font-bold border", status.bg, status.color, "border-current/10")}>
            ...
            <div className="flex items-center bg-white border border-black/10 rounded p-1">
              <button
                ...
                className={cn(
                  ...
                  viewMode === "list" ? "bg-foreground text-background" : "text-muted-foreground hover:bg-black/5"
                )}
              >
                <List size={14} />
              </button>
              <button
                ...
                className={cn(
                  "p-1.5 rounded transition-all",
                  viewMode === "board" ? "bg-foreground text-background" : "text-muted-foreground hover:bg-black/5"
                )}
              >
          ...
          {viewMode === "list" ? (
            <div className="grid grid-cols-1 gap-3">
              {sortedIssues.map(issue => (
                <IssueCard key={issue.id} issue={issue} isSelected={selectedIssues.has(issue.id)} />
              ))}
              {sortedIssues.length === 0 && (
                <div className="py-20 text-center border border-dashed border-black/10 rounded">
                   <p className="text-[12px] font-bold text-muted-foreground">No active issues found in this workspace.</p>
                </div>
              )}
            </div>
          ) : (
            <div className="flex gap-6 overflow-x-auto pb-4 no-scrollbar">
              {Object.entries(boardGroups).map(([title, issues]) => {
                const config = Object.values(STATUS_CONFIG).find(s => s.boardColumn === title);
                return (
                  <div key={title} className="flex-1 min-w-[300px] space-y-4">
                    <div className="flex items-center justify-between px-1">
                      <div className="flex items-center gap-2">
                        <config.icon size={13} className={config.color} strokeWidth={3} />
                        <h3 className="text-[12px] font-bold text-foreground uppercase tracking-widest">{title}</h3>
      ...
      {selectedIssues.size > 0 && (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 animate-in slide-in-from-bottom-4">
          <div className="bg-foreground text-background px-5 py-2.5 rounded shadow-2xl flex items-center gap-4 border border-white/10">
            <span className="text-[11px] font-bold">{selectedIssues.size} Strategic Items Selected</span>
            <div className="w-px h-4 bg-white/20" />
            <button className="text-[11px] font-bold hover:text-white/80 transition-colors">Archive</button>
            <button 
